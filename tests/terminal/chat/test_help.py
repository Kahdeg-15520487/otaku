...
from otaku.terminal.chat.bindings import help_text
...
class TestFits:
    def test_no_line_outruns_the_width(self) -> None:
        for width in WIDTHS:
            longest = max(len(line) for line in help_text(width).splitlines())
...
class TestColumns:
    def test_a_wide_terminal_takes_two_columns(self) -> None:
        assert len(help_text(200).splitlines()) < len(help_text(120).splitlines()) * 0.8
    ...
    def test_a_narrow_terminal_stays_in_one(self) -> None:
        ...
        assert max(len(line) for line in help_text(80).splitlines()) <= 80
        assert "Playing:" in help_text(80).splitlines()[0]

    def test_growing_the_terminal_never_costs_height(self) -> None:
        heights = [len(help_text(width).splitlines()) for width in WIDTHS]
