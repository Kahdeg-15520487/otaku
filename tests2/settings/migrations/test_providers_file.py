...
from otaku2.settings.migrations.providers_file import move_providers_text, seal_api_keys
...
class TestSealApiKeys:
    def test_seals_every_plain_key(self) -> None:
        text = '[a]\napi_key = "plain-1"\n\n[b]\nurl = "y"\napi_key = "plain-2"\n'
        migrated = seal_api_keys(lambda v: f"sealed:{v}", _is_sealed)(text)
        assert migrated == (
    ...
    def test_sealed_and_empty_keys_stay(self) -> None:
        text = '[a]\napi_key = "sealed:abc"\n\n[b]\napi_key = ""\n'
        assert seal_api_keys(lambda v: "BOOM", _is_sealed)(text) is text

    def test_a_seal_that_cannot_happen_leaves_the_line_for_next_time(self) -> None:
        text = '[a]\napi_key = "plain"\n'
        assert seal_api_keys(lambda v: v, _is_sealed)(text) is text


def _is_sealed(value: str) -> bool:
    return value.startswith("sealed:")
