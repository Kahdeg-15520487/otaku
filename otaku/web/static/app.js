/* The page over an open session: what it asks for at boot, and every
   ...
   - A command is reachable by click and by typing, and both go through
     `commands.run`. A button carries the token, never behaviour of its
     own — and never its label as a decision, which is why every one of
     them has a `data-` attribute instead.
...
function start() {
  ...
  watchServer(boot);
  $(".otk-banner [data-retry]")?.addEventListener("click", boot);

  // Every command button, wherever it is: the rail, a popup's header,
  // the turn bar under the last exchange.
  document.addEventListener("click", (event) => {
    const command = event.target.closest("[data-command]");
    if (command) {
      /* A command carried by a popup's own header — the lore browser's
         "Extract now" — answers into the flow, which is behind the
         modal. Close first, so the reader sees what it did. Unless it
         ASKS first: a question belongs over the screen it was asked
         from, and a cancelled one must leave that screen exactly as it
         was (`commands.confirmed` closes them once it is answered). */
      if (command.closest("dialog[open]") && !asksFirst(command.dataset.command)) closeAll();
      runCommand(command.dataset.command);
    }
    const turn = event.target.closest("[data-turn]");
    if (turn) runCommand(turn.dataset.turn === "undo" ? "/undo" : "/regen");
  });
