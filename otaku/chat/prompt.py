"""The prompt: everything between the user's keyboard and one submitted line.

`build_prompt` assembles the prompt_toolkit session — store-backed
history, the slash-command menu, the keybindings — and `LineAssembler`
turns the lines it reads into submissions. The run loop (repl.py) only
prompts, feeds, and submits; every prompt_toolkit decision lives here.
...
the in-progress text in `Carry` so the next prompt restores it.
