...
from otaku2.encryption import unseal
from otaku2.terminal.screens import models as screen_models
from otaku2.terminal.tty import clipboard
from scenarios2.support import server as scripted
from scenarios2.support.harness import App, launch, set_config_provider
from scenarios2.support.screens import CTRL_V, ENTER, ESC, pasted, run_screen
...
class TestCloudProviders:
    ...
    def test_the_catalog_lists_with_context_and_no_sizes(self, tmp_path) -> None:
        ...
        try:
            ...
            try:
                rows, _ = api_providers.get_providers(app.session)
                catalog = next(r for r in rows if r.config.name == "openrouter")
    ...
    def test_the_picker_does_not_wait_for_the_catalogs(self, server, tmp_path) -> None:
        ...
        try:
            ...
            assert all(row.config.name != "openrouter" for row in rows)
    ...
    def test_nanogpt_asks_for_the_detailed_listing(self, tmp_path) -> None:
        ...
        try:
            ...
            try:
                rows, _ = api_providers.get_providers(app.session)
                nano = next(r for r in rows if r.config.name == "nanogpt")
                assert [m.context for m in nano.models] == [64_000]
            finally:
                app.close()
        finally:
            server.close()
