...
from otaku2.backend.session import Session
from otaku2.encryption import seal
...
from otaku2.terminal.chat import loop
from otaku2.terminal.chat.chat import Chat
...
def run_otaku(root: Path, *args: str) -> subprocess.CompletedProcess[str]:
    ...
    return subprocess.run(
        [sys.executable, "-c", "from otaku2.cli import main; main()", *args],
        capture_output=True,
        text=True,
        timeout=30,
...
def set_config_provider(
    ...
    *,
    ...
    providers = (
        providers_file.load(paths.providers_file) if paths.providers_file.exists() else {}
    )
