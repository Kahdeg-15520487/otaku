"""Plain text, dismantled into verbatim narration turns — the open
class: it never rejects a format, so the caller routes here only when
no marked format claimed the file.
...
from otaku2.backend.formats import ExportedMessage, StoryExport
