"""The derivative memory: scenes, cast, journals — one ops class per
table.

Everything here is distilled from the source by lore extraction and can
be rebuilt from it. The write disciplines are part of the data model
(see `store.schema`); each edit writer below carries its own
invalidation, so correcting a per-scene record nulls the rollups
composed from it and the next pass rebuilds them.
"""
...
from otaku2.store.database import Database
from otaku2.store.schema import Character, Journal, Scene
